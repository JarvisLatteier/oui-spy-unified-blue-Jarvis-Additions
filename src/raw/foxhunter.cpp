...
<body>
    ...
    display_foxhunter(-100, targetMAC.c_str(), false, "CONFIG");
#endif
}
...
#ifdef ENABLE_TFT_DISPLAY
    display_foxhunter(-100, targetMAC.c_str(), false, "SEARCHING");
#endif
}
...
#ifdef ENABLE_TFT_DISPLAY
                display_foxhunter(currentRSSI, targetMAC.c_str(), true, "TRACKING");
#endif
#if HAS_SD_CARD
                { char buf[128];
                  snprintf(buf, sizeof(buf),
                    "{\"ts\":%lu,\"mac\":\"%s\",\"rssi\":%d,\"state\":\"TRACKING\"}",
                    millis(), targetMAC.c_str(), currentRSSI);
                  sdlog_write(2, buf); }
#endif
            }
