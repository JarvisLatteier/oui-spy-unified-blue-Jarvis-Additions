...
#endif
#ifndef NEOPIXEL_DETECTION_BRIGHTNESS
...
#elif HAS_APA102
// APA102 alternatives — delegate to led_strip driver
void initializeNeoPixel() {
    ledStrip_init();
}

void updateNeoPixelAnimation() {
    // APA102 breathing handled by led_strip
    static unsigned long lastUpdate = 0;
    static float brightness = 0.0;
    static bool increasing = true;
    if (detectionMode) {
    ...
    if (millis() - lastUpdate >= 20) {
        lastUpdate = millis();
        if (increasing) { brightness += 0.02; if (brightness >= 1.0) { brightness = 1.0; increasing = false; } }
        else { brightness -= 0.02; if (brightness <= 0.1) { brightness = 0.1; increasing = true; } }
        uint8_t v = (uint8_t)(50 * brightness);
        ledStrip_setColor(v, 0, v);  // Pink breathing
        ledStrip_show();
    }
}

void setNeoPixelColor(uint8_t r, uint8_t g, uint8_t b) {
    ledStrip_setColor(r, g, b);
    ledStrip_show();
}
...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        .br-panel { margin-top: 15px; padding: 12px; background: rgba(255,20,147,0.05); border: 1px solid rgba(255,20,147,0.15); border-radius: 8px; }
        .br-panel h4 { color: #ff1493; margin-bottom: 10px; font-size: 14px; }
        .br-panel label { display: block; color: #ccc; font-size: 13px; margin-bottom: 4px; }
        .br-panel input[type=range] { width: 100%; margin-bottom: 8px; accent-color: #ff1493; }
        .br-panel .val { color: #ff1493; font-weight: bold; }
        .br-panel .incog { margin-top: 8px; display: flex; align-items: center; gap: 8px; }
        .br-panel .incog input { accent-color: #ff1493; width: 16px; height: 16px; }
    </style>
</head>
<body>
...
11:22:33:44:55:66">%MAC_VALUES%</textarea>
            ...
            <div class="section">
                ...
                <div class="br-panel">
                    ...
                    <input type="range" id="dspBr" min="0" max="255" value="200" oninput="document.getElementById('dspVal').textContent=this.value" onchange="setBr()">
                    <label>LED Strip: <span class="val" id="ledVal">--</span></label>
                    <input type="range" id="ledBr" min="0" max="255" value="128" oninput="document.getElementById('ledVal').textContent=this.value" onchange="setBr()">
                    <div class="incog">
                        <input type="checkbox" id="incog" onchange="setIncog(this.checked)">
                        <label style="margin:0;display:inline">Incognito Mode (kill display + LED)</label>
                    </div>
                </div>
                <script>
                function setBr(){fetch('/brightness?disp='+document.getElementById('dspBr').value+'&led='+document.getElementById('ledBr').value);}
                function setIncog(on){var d=on?0:document.getElementById('dspBr').value,l=on?0:document.getElementById('ledBr').value;fetch('/brightness?disp='+d+'&led='+l);}
                fetch('/brightness').then(r=>r.json()).then(j=>{document.getElementById('dspBr').value=j.disp;document.getElementById('dspVal').textContent=j.disp;document.getElementById('ledBr').value=j.led;document.getElementById('ledVal').textContent=j.led;});
                </script>
            </div>
...
<body>
    ...
    display_detector_scanning((int)targetFilters.size(), 0);
#endif
}
...
#ifdef ENABLE_TFT_DISPLAY
                        display_detector_event(mac.c_str(), rssi,
                            matchedDescription.c_str(), "RE-30s", (int)devices.size());
#endif
#if HAS_SD_CARD
                        { char buf[128];
                          snprintf(buf, sizeof(buf),
                            "{\"ts\":%lu,\"mac\":\"%s\",\"rssi\":%d,\"type\":\"RE-30s\"}",
                            millis(), mac.c_str(), rssi);
                          sdlog_write(1, buf); }
#endif
                        threeBeeps();
                        dev.inCooldown = true;
...
#ifdef ENABLE_TFT_DISPLAY
                        display_detector_event(mac.c_str(), rssi,
                            matchedDescription.c_str(), "RE-3s", (int)devices.size());
#endif
#if HAS_SD_CARD
                        { char buf[128];
                          snprintf(buf, sizeof(buf),
                            "{\"ts\":%lu,\"mac\":\"%s\",\"rssi\":%d,\"type\":\"RE-3s\"}",
                            millis(), mac.c_str(), rssi);
                          sdlog_write(1, buf); }
#endif
                        twoBeeps();
...
#ifdef ENABLE_TFT_DISPLAY
                display_detector_event(mac.c_str(), rssi,
                    matchedDescription.c_str(), "NEW", (int)devices.size());
#endif
#if HAS_SD_CARD
                { char buf[128];
                  snprintf(buf, sizeof(buf),
                    "{\"ts\":%lu,\"mac\":\"%s\",\"rssi\":%d,\"type\":\"NEW\"}",
                    millis(), mac.c_str(), rssi);
                  sdlog_write(1, buf); }
#endif
                threeBeeps();
...
#ifdef ENABLE_TFT_DISPLAY
            if (!newMatchFound) {
                display_detector_scanning((int)targetFilters.size(), (int)devices.size());
            }
#endif
        }
